...
URL = "https://jeodpp.jrc.ec.europa.eu/ftp/jrc-opendata/GHSL/GHS_BUILT_S_GLOBE_R2023A/GHS_BUILT_S_E2020_GLOBE_R2023A_4326_3ss/V1-0/GHS_BUILT_S_E2020_GLOBE_R2023A_4326_3ss_V1_0.zip"
...
def extract_archive(archive_path: Path, raw_dir: Path) -> Path:
    ...
    if marker.exists():
        tif = next(raw_dir.rglob("*.tif"), None)
        if tif is not None:
            return tif
...
def main() -> int:
    ...
    write_json(output_dir / "ready.json", {"raster": str(tif_path)})
    print(json.dumps({"raster": str(tif_path)}, ensure_ascii=False, indent=2))
    return 0
